# analysis with non-parametric propensity score
# 22.05.2019
...
non.parametric <- function(){
    ...
    for (e in 1: N_imputations){
      ...
      data.train <- df %>%
        dplyr::select(
          ...
          salience.relative, scom, con_of_int, ctte_op,
  ...
  # saving
  setwd(subdirs$workdata)
  box_gb <- list(outs, sds, rsqrt, N = obs)
  save(box_gb, file = "non_parametric_model.RData")
  save(df.matched2, file = "df.matched2 relative salience.RData")
  #save(S.combined, file = "S.combined_non_parametric.RData")
}
